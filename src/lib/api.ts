...
export interface Config {
  ...
  decorations: { title: string; context: string; group?: string | null }[];
  configured: boolean; config_path: string;
}
export const desktop = isTauri();
export const defaults: Config = {
  ai: { provider: 'openai', model: 'gpt-5.6-terra', base_url: 'https://api.openai.com/v1' },
  general: { clipboard_auto_paste: false, auto_copy_result: false, default_n: 1 },
  ...
  ], decorations: [], configured: false, config_path: '~/.config/toneweave/config.yaml',
};
export const getConfig = () => desktop ? invoke<Config>('get_config') : Promise.resolve(defaults);
export async function generate(source: string, direction: string, preset: string, greeting: boolean, n: number, decorations: string[]) {
  if (!desktop) throw new Error('Open the Toneweave desktop app to generate replies. Run pnpm tauri dev.');
  return invoke<Generation>('generate_reply', { source, direction, preset, greeting, n, decorations });
}
...
export interface Draft { source: string; direction: string; preset: string; greeting: boolean; count: number; decorations: string[]; replies: Reply[] }
export const saveDraft = (draft: Draft) => desktop ? invoke<void>('save_draft', { content: JSON.stringify(draft) }) : Promise.resolve();
export const clearDraft = () => desktop ? invoke<void>('clear_draft') : Promise.resolve();
export async function loadDraft(): Promise<Partial<Draft> | null> {
  if (!desktop) return null;
  const text = await invoke<string | null>('load_draft');
  if (!text) return null;
  try {
